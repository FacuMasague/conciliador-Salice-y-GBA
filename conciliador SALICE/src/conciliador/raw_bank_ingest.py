...
def _parse_mp_raw_xml(path: str) -> List[RawBankTxn]:
    ...
    for row in data_rows:
        fecha, _hora = _parse_datetime(row[fecha_idx] if fecha_idx < len(row) else None)
...
def _parse_mp_raw(path: str) -> List[RawBankTxn]:
    ...
    out: List[RawBankTxn] = []
    for _, row in df.iterrows():
        fecha, _hora = _parse_datetime(row.get(fecha_col))
        importe = _parse_amount(row.get(importe_col))
        if not fecha or importe is None or importe <= 0:
            continue
        detalle = str(row.get(detalle_col) or "").strip()
...
def _pick_bbva_sheet(wb: openpyxl.Workbook, source_name: str) -> Optional[str]:
    if "SALICE BBVA" in wb.sheetnames and " ALARCON BBVA" not in wb.sheetnames:
        return "SALICE BBVA"
    if " ALARCON BBVA" in wb.sheetnames and "SALICE BBVA" not in wb.sheetnames:
        return " ALARCON BBVA"
    if "SALICE BBVA" in wb.sheetnames and " ALARCON BBVA" in wb.sheetnames:
        n = _norm(source_name)
        if "alarcon" in n:
            return " ALARCON BBVA"
        if "salice" in n:
...
def _pick_mp_sheet(wb: openpyxl.Workbook) -> Optional[str]:
...
def _append_bbva(ws: openpyxl.worksheet.worksheet.Worksheet, txns: List[RawBankTxn]) -> tuple[int, int]:
    header_row = _find_header_row(ws, ["Número Documento", "Importe"]) or _find_header_row(ws, ["Numero Documento", "Importe"])
    if header_row is None:
        raise ValueError(f"No pude detectar encabezado BBVA en hoja '{ws.title}'.")

    amount_col = _find_col_idx(ws, header_row, ["Importe"]) or 5
    ...
    existing = _existing_counter_bbva(ws, header_row, amount_col)
    txns_sorted = sorted(txns, key=lambda t: (t.fecha, round(float(t.importe), 2), _norm_for_sig(t.detalle)))
    append_row = _next_append_row(ws, header_row=header_row, key_cols=[1, 2, amount_col])
    skipped = 0
    added = 0
    for t in txns_sorted:
        ...
        if existing[sig] > 0:
            ...
            skipped += 1
            continue
        ws.cell(append_row, 1, t.fecha.strftime("%d-%m-%Y"))
...
def _append_mp(ws: openpyxl.worksheet.worksheet.Worksheet, txns: List[RawBankTxn]) -> tuple[int, int]:
    ...
    tipo_col = _find_col_idx(ws, header_row, ["Tipo de Operación", "Tipo de Operacion"])
    ...
    # Si la plantilla no tiene columna de CUIT del pagador, la agregamos al final.
    if cuit_col is None:
        cuit_col = int(ws.max_column) + 1
        ws.cell(header_row, cuit_col, "NÚMERO DE IDENTIFICACIÓN DEL PAGADOR")

    existing = _existing_counter_mp(ws, header_row, fecha_col, detalle_col, importe_col)
    txns_sorted = sorted(txns, key=lambda t: (t.fecha, round(float(t.importe), 2), _norm_for_sig(t.detalle)))
    ...
    for t in txns_sorted:
        ...
        if tipo_col:
            ws.cell(append_row, tipo_col, "Cobro")
    ...
    *,
    ...
    if detected_rows.get("BBVA"):
        if bbva_sheet_default is None:
            raise ValueError("El consolidado no tiene hoja BBVA compatible ('SALICE BBVA' o ' ALARCON BBVA').")
    ...
    if detected_rows.get("MERCADOPAGO"):
        mp_sheet = _pick_mp_sheet(wb)
        if mp_sheet is None:
            raise ValueError("El consolidado no tiene hoja MercadoPago compatible.")
        ws = wb[mp_sheet]
        added, skipped = _append_mp(ws, detected_rows["MERCADOPAGO"])
        summary["MERCADOPAGO"]["appended"] = int(added)
        summary["MERCADOPAGO"]["duplicates_skipped"] = int(skipped)
        summary["MERCADOPAGO"]["sheet"] = mp_sheet
        if added > 0:
