...
def _fetch_getitem_details(
    *,
    ...
    request_id_last: str | None = None
    rate_limited = False
    retry_429 = int((os.getenv("RECEIPTS_API_GETITEM_429_RETRIES", "1") or "1").strip())
    wait_429 = int((os.getenv("RECEIPTS_API_GETITEM_429_WAIT_SECONDS", "65") or "65").strip())
...
def _fetch_medios_pago(
    *,
    ...
    for target in targets or ["3"]:
        ...
        try:
            page_rows, rid, _ok = _fetch_paged_rows(
                base=base,
                path=medios_path,
                method="GET",
                headers=headers,
...
def _get_paged_get(
    *,
    ...
    while True:
        url = f"{base}{path}?{urlencode({'pageNumber': str(page), 'pageSize': str(page_size)})}"
        payload, rid = _http_json(url, method="GET", headers=headers)
        if rid:
            request_id_last = rid
        page_rows = _extract_rows(payload, list_key)
        if page_rows is None:
            warnings.append(f"Respuesta sin lista '{list_key}' en {path} (page={page}); se toma vacío.")
            page_rows = []
        rows.extend(page_rows)
        ...
        page += 1
        if page > 500:
            warnings.append(f"Corte de paginación de seguridad en {path}")
            break
...
def fetch_receipts_payload(
    *,
    ...
    page_size = int((os.getenv("RECEIPTS_API_PAGE_SIZE", "500") or "500").strip())
    ...
    for target in targets:
        ...
        for method in ("POST", "GET"):
            for path in comprobantes_paths:
                ...
                try:
                    rows, rid, _ok = _fetch_paged_rows(
                        base=base,
                        path=path,
                        method=method,
                        headers=headers,
    ...
    if not operational_rows:
        ...
        if comprobantes:
            ...
            if keys and not rate_limited:
                ...
                ratio = len(detail_by_key) / max(len(all_keys), 1)
                adaptive_limit = int((os.getenv("RECEIPTS_API_GETITEM_ADAPTIVE_MAX_KEYS", "1200") or "1200").strip())
                ...
                missing = [k for k in keys if _normalize_getitem_key_tuple(k) not in detail_by_key]
                extra_passes = int((os.getenv("RECEIPTS_API_GETITEM_EXTRA_PASSES", "2") or "2").strip())
                extra_chunk = int((os.getenv("RECEIPTS_API_GETITEM_EXTRA_CHUNK_SIZE", "10") or "10").strip())
                extra_chunk = min(max(extra_chunk, 1), 60)
                extra_wait = float((os.getenv("RECEIPTS_API_GETITEM_EXTRA_WAIT_SECONDS", "1") or "1").strip())
