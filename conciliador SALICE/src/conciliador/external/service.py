...
def _repartidor_id_from_comprobante(row: Dict[str, Any]) -> str:
    value = _norm_numeric_id(
        _get_any(row, "repartidorID", "repartidor_id", "RepartidorID", "idRepartidor")
    )
    return "" if value == "0" else value


def _repartidores_lookup(empresa_filter: str | None = None) -> tuple[Dict[str, str], list[str]]:
    try:
        response = fetch_repartidores_payload(empresa_filter=empresa_filter)
    except Exception as exc:
        return {}, [f"No se pudo obtener el maestro de repartidores: {exc}"]

    lookup: Dict[str, str] = {}
    for row in response.payload.get("repartidores") or []:
        if not isinstance(row, dict):
            continue
        repartidor_id = _norm_numeric_id(
            _get_any(row, "repartidorID", "repartidor_id", "RepartidorID", "id")
        )
        ...
        if repartidor_id and repartidor_id != "0" and nombre:
            lookup[repartidor_id] = nombre
...
def _cliente_features_lookup(
    ...
    cliente_ids = list(
        dict.fromkeys(
            ...
            if (cliente_id := _norm_numeric_id(
    ...
    if not cliente_ids:
        return {}, {}, []
    try:
        # GetList evita disparar cientos de GetItem y respeta el límite de 60
        # solicitudes/minuto del tenant MDP. Filtramos localmente por los clientes
    ...
    wanted = set(cliente_ids)
    for row in response.payload.get("clientes") or []:
        if not isinstance(row, dict):
            continue
...
def _comprobante_empresa_id(c: Dict[str, Any]) -> str:
    # Prioridad de negocio actual: sucursal_id define empresa (3=SALICE, 6=ALARCON).
    for v in (
...
def _gesi_nro_recibo_from_comprobante(c: Dict[str, Any]) -> Optional[str]:
    # Regla operativa: solo tomar recibos con número de Pedidos Móviles (PM).
    # 1) Prioridad: campo explícito nro_recibo_pm.
    nro_pm = _normalize_recibo_pm(_get_any(c, "nro_recibo_pm", "nroReciboPm", "NroReciboPm"))
    if nro_pm:
        return nro_pm

    # 2) Fallback: codigoDeImportacion únicamente si evidencia marker PM.
    #    Ejemplos válidos: PMCBR_69016, PM_12345.
    ...
    # Casos válidos:
    # - contiene marcador PM explícito (ej: PMCBR_69016)
    # - o viene directo numérico (algunas instalaciones ya devuelven solo el nro PM)
    if "pm" in t:
...
def fetch_receipts_and_payments(
    ...
    # Contract option C (GESI): payload.comprobantes[] + payload.formasDePago[]
        ...
        skipped_without_nro_pm = 0
        skipped_unsupported_forma = 0
        for i, it in enumerate(comprobantes):
            ...
            if not is_bancarizable:
                skipped_unsupported_forma += 1
                continue
            ...
            payments_flat.append(
                ExternalPayment(
                    ...
                    importe_pago=importe_bankable
                    if importe_bankable is not None
                    else _parse_float(
        ...
        if skipped_without_nro_pm > 0:
            resp.warnings.append(
                f"Se omitieron {skipped_without_nro_pm} comprobantes sin nro de recibo PM utilizable."
            )
        if skipped_unsupported_forma > 0:
            resp.warnings.append(
                f"Se omitieron {skipped_unsupported_forma} comprobantes con forma de pago no bancaria."
            )
    else:
    ...
    if not payments_flat:
        meta = {
            ...
            "cliente_cuit_map": cliente_cuit_map,
        }
        return [], meta
...
def fetch_cliente_cuit_map(empresa_filter: str | None = None) -> Tuple[Dict[str, str], Dict[str, Any]]:
    resp = fetch_padron_payload(empresa_filter=empresa_filter)
    payload = _expect_obj(resp.payload, "payload")
