...
def test_fetch_cliente_cuit_map_maps_entries(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.conciliador.external.service.fetch_padron_payload",
        lambda empresa_filter=None: _Resp(payload, request_id="r-pad"),
    )
...
def test_fetch_cliente_cuit_map_skips_invalid_rows_if_some_valid(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.conciliador.external.service.fetch_padron_payload",
        lambda empresa_filter=None: _Resp(payload, request_id="r-pad-2"),
    )
...
def test_fetch_receipts_and_payments_maps_gesi_comprobantes(monkeypatch):
    ...
    assert payments[0].nro_recibo == "69016"
...
def test_fetch_receipts_and_payments_gesi_skips_when_nro_recibo_pm_missing(monkeypatch):
    payload = {
        "comprobantes": [
            {
                "empresaID": 6,
    ...
    assert any("no devolvió pagos válidos" in w for w in meta.get("external_warnings", []))
...
def test_fetch_receipts_and_payments_skips_non_pm_codigo_importacion(monkeypatch):
    ...
    assert payments == []
    assert meta["payments_count"] == 0
    assert any("sin nro de recibo PM" in w for w in meta.get("external_warnings", []))
...
def test_fetch_receipts_and_payments_skips_non_bankable_efectivo(monkeypatch):
    payload = {
        "comprobantes": [
            {
                "empresaID": 3,
    ...
    assert any("forma de pago no bancaria" in w for w in meta.get("external_warnings", []))
...
def test_fetch_receipts_and_payments_uses_medios_de_pago_by_valor_id(monkeypatch):
    ...
    payments, meta = fetch_receipts_and_payments(15, None)
    # Solo queda el bancarizable (valorID=2 tipo=B)
    assert len(payments) == 1
    assert payments[0].nro_recibo == "70122"
    assert "Transf. Bancaria" in payments[0].medio_pago
    assert meta["medio_bancarizable_stats"]["BANKABLE"] >= 1
    assert meta["medio_bancarizable_stats"]["NON_BANKABLE"] >= 1
